...
BOT_TOKEN = "8884774315:AAH-AJ7R8gsM2rIsXVcKVoY4cf7rhwqxdmo"

ADMIN_ID = 8242348109

# ─── EXTERNAL API (YesMS) ─────────────────────────────────────────────────────
YESMS_BASE = "https://yesms.online/api"

# ─── PANEL BASE URLS ──────────────────────────────────────────────────────────
STEXSMS_BASE   = "https://api.2oo9.cloud/MXS47FLFX0U/tness/@public/api"
FASTXOTPS_BASE = "https://2eee7.com/@Access/@Bot/2eee7/@public"
VOLTXSMS_BASE  = "https://api.2oo9.cloud/MXS47FLFX0U/tnevs/@public/api"

# ─── API CREDENTIALS (URLs only; keys managed entirely via admin panel) ────────
SMSHADI_URL = "http://147.135.212.197/crapi/had/viewstats"
...
def _powered_by() -> str:
    return get_setting("powered_by", "TARIQUL ᯓᡣ𐭩")
...
if not BOT_TOKEN:
    print("❌ BOT_TOKEN পাওয়া যায়নি! .env ফাইলে BOT_TOKEN=your_token সেট করুন।")
    import sys; sys.exit(1)
if not ADMIN_ID:
    print("❌ ADMIN_ID পাওয়া যায়নি! .env ফাইলে ADMIN_ID=your_user_id সেট করুন।")
    import sys; sys.exit(1)
...
# ─── DATABASE ──────────────────────────────────────────────────────────────────
def get_conn():
    conn = sqlite3.connect("voltx.db", check_same_thread=False)
...
def welcome_keyboard(is_admin_user=False):
    ...
    # Row 1 — Primary action + wallet (green = go, yellow = money)
    kb.add(
        KeyboardButton(f"🟢 📲 {stylish('Get Number')}"),
        KeyboardButton(f"🟡 💰 {stylish('Balance')}"),
    )
    # Row 2 — Custom range (full width, purple star)
    kb.add(KeyboardButton(f"🟣 🎯 {stylish('CUSTOM RANGE')}"))
    # Row 3 — Support + withdraw (blue = help, orange = cash)
    kb.add(
        KeyboardButton(f"🔵 🛡️ {stylish('Support')}"),
        KeyboardButton(f"🔴 💸 {stylish('Withdraw')}"),
    )
    # Row 4 — Profile + live traffic (white = identity, red = live)
    kb.add(
        KeyboardButton(f"🟠 ✨ {stylish('Profile')}"),
        KeyboardButton(f"🔥 📡 {stylish('Traffic')}"),
    )
    # Row 5 — Leaderboard + refer (trophy gold, gift pink)
    kb.add(
        KeyboardButton(f"🥇 🏆 {stylish('Leaderboard')}"),
...
def number_card_inline_keyboard(number: str = ""):
    ...
    if number:
        safe_num = str(number).strip().lstrip("+")
        display = f"+{safe_num}"
        label = f"📋 {stylish('COPY NUMBER')}  {display}"
...
# ─── MESSAGE BUILDERS ──────────────────────────────────────────────────────────
def build_number_card(flag, code, name, number, service):
    pw = stylish(_powered_by())
    sep = "━" * 32
    num_str = str(number)
    # Large prominent number block — easy to read and long-press copy
    return (
        f"🟢 {sep}\n"
        ...
        f"</blockquote>\n\n"
        f"📞 {stylish('YOUR NUMBER')}\n"
        f"┌{'─'*30}┐\n"
        f"│  <code>+{num_str}</code>\n"
        f"└{'─'*30}┘\n"
        f"<i>👆 Tap the number above to copy it instantly</i>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🕷 {stylish('POWERED BY')} <b>{pw}</b>"
    )
...
def build_timeout_card(flag, code, name, number, service):
...
def build_profile_text(user, db_user, stats):
    sep = "━" * 28
    joined = datetime.fromtimestamp(db_user["joined_at"]).strftime("%d %b %Y") if db_user.get("joined_at") else "N/A"
    return (
        f"👤 {sep}\n"
        ...
        f"◆ 📞 {stylish('Numbers')}    ▸  <b>{db_user['numbers_generated']}</b>\n"
        f"◆ 🔢 {stylish('OTPs Got')}   ▸  <b>{db_user['otps_received']}</b>\n\n"
        f"◆ 💳 {stylish('Balance')}    ▸  <b>{stats['balance']:.2f} BDT</b>\n"
...
def assign_number_to_user(user_id, chat_id, country_id):
    ...
    with get_conn() as conn:
        ...
        if not number_row:
            bot.send_message(
                chat_id, f"❌ {stylish('No numbers available for this country.')}",
                reply_markup=welcome_keyboard(is_admin_user=is_admin(user_id)),
            )
            return
        ...
        msg = bot.send_message(chat_id, text, reply_markup=number_card_inline_keyboard(number=str(number_row["number"])))
...
def _assign_range_number_to_user(user_id, chat_id, country, service, range_id):
    ...
    num_data = fetch_api_number(range_id)
    if not num_data:
        retry_kb = InlineKeyboardMarkup()
        retry_kb.add(InlineKeyboardButton(f"🔄 {stylish('Try Again')}", callback_data=f"custom_range_retry:{range_id}"))
        try:
            bot.edit_message_text(
                f"❌ {stylish('No numbers available for')} {_html.escape(country['name'])}. {stylish('Please try again later.')}",
                chat_id=chat_id,
    ...
    text_card = build_number_card(country["flag"], country["code"], country["name"], full_number, service["name"])
    final_message_id = loading_msg.message_id
    try:
        bot.edit_message_text(
            text_card, chat_id=chat_id, message_id=loading_msg.message_id,
            reply_markup=number_card_inline_keyboard(number=str(full_number)),
        )
    except Exception:
        sent = bot.send_message(chat_id, text_card, reply_markup=number_card_inline_keyboard(number=str(full_number)))
...
def handle_text(message):
    ...
    if is_admin(user.id):
        ...
        # ── OTHERS LINK menu (Task 4) ──────────────────────────────────────
            ...
            pw = get_setting("powered_by", "TARIQUL ᯓᡣ𐭩")
            bot.send_message(
                chat_id,
            ...
            cur = get_setting("powered_by", "TARIQUL ᯓᡣ𐭩")
            bot.send_message(
                chat_id,
        ...
        if text in _SKIP_BUTTONS:
            ...
            if not num_data:
                try:
                    ...
                    bot.edit_message_text(
                        f"❌ {stylish('No numbers available for range')} <code>{rid}</code>. {stylish('Please try another range.')}",
                        chat_id=chat_id,
                        message_id=loading_msg.message_id,
                        reply_markup=retry_kb,
                    )
            ...
            api_country_raw = num_data.get("country") or ""
            if api_country_raw:
                flag, api_country = extract_flag_from_name(api_country_raw)
                if flag == "🌐":
                    country_with_flag = range_to_country_name(rid)
                    flag, _ = extract_flag_from_name(country_with_flag)
            else:
                country_with_flag = range_to_country_name(rid)
                flag, api_country = extract_flag_from_name(country_with_flag)
            text_card = build_number_card(flag, country_code, api_country, full_number, service_name)
            try:
                ...
                sent = bot.send_message(chat_id, text_card, reply_markup=number_card_inline_keyboard())
            with get_conn() as conn:
                conn.execute(
                    """INSERT INTO allocations
                       (user_id, number_id, number, service_name, country_name,
    ...
    if text in (f"🟢 📲 {stylish('Get Number')}", f"🟢 📲 {stylish('Get Another Number')}"):
        ...
        db_user = get_user(user.id)
        stats = get_wallet_stats(user.id)
        bot.send_message(chat_id, build_profile_text(user, db_user, stats), reply_markup=welcome_keyboard(is_admin_user=is_admin(user.id)))
        return
...
def handle_document(message):
    ...
    if state.get("step") == "abackup_restore_wait":
        ...
        try:
            ...
            _shutil.copy2(tmp_path, "voltx.db")
            import os as _os
            _os.remove(tmp_path)
        except Exception as e:
...
def _otpwork_fetch_number(call, entry: dict, rid: str, send_new: bool = False):
    ...
    if not num_data:
        ...
        if not send_new:
            try:
                bot.edit_message_text(
                    f"❌ {stylish('No numbers available for')} {_html.escape(country_name)}. {stylish('Please try another.')}",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=retry_kb,
                )
    ...
    api_country = num_data.get("country") or clean_name
    ...
    if send_new:
        ...
        msg = bot.send_message(
            call.message.chat.id, text_card, reply_markup=number_card_inline_keyboard()
        )
    else:
...
def cb_api_country_sel(call):
    ...
    if not num_data:
        try:
            bot.edit_message_text(
                f"❌ {stylish('No numbers available for')} {country_name}. {stylish('Please try another country.')}",
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                reply_markup=None,
            )
