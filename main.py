...
def is_demo_sms_enabled() -> bool:
    return str(get_setting("demo_sms_enabled", "0")) == "1"
...
def settings_keyboard(is_main_admin=False):
    ...
    kb.add(KeyboardButton(f"🧪 {stylish('Auto SMS Demo')}"))
    kb.add(KeyboardButton(f"🗄 {stylish('Backup')}"))
    kb.add(KeyboardButton(f"🔙 {stylish('Back to Admin')}"))
    return kb


def demo_sms_keyboard():
    """Auto SMS Demo sub-menu (admin)."""
    kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    state = "OFF" if is_demo_sms_enabled() else "ON"
    kb.add(KeyboardButton(f"🧪 {stylish('Demo')}: {state}"))
    kb.add(
        KeyboardButton(f"💬 {stylish('Set Demo Group ID')}"),
        KeyboardButton(f"⏱ {stylish('Set Demo Interval')}"),
    )
...
def otp_group_keyboard():
    """Inline keyboard for the OTP GROUP forward message — NUMBER PANEL + BOT DEVELOPER."""
    panel_link = get_setting("panel_link", "")
...
def handle_text(message):
    ...
    if is_admin(user.id):
        ...
        if user.id in admin_states:
            ...
            if text == f"❌ {stylish('Cancel')}":
                ...
                _ol_kb_steps = ("aset_ol_support", "aset_ol_otpgroup", "aset_ol_payment_id", "aset_ol_otp_fwd")
            ...
            # ── AUTO SMS DEMO SETTINGS ────────────────────────────────────────
            if step == "ademo_chat_id":
                raw = text.strip()
                if not re.fullmatch(r"-?\d{5,20}", raw):
                    bot.send_message(chat_id, "❌ Invalid chat ID.", reply_markup=cancel_keyboard())
                    return
                set_setting("demo_sms_chat_id", raw)
                admin_states.pop(user.id, None)
                bot.send_message(chat_id, _demo_sms_status_text(), reply_markup=demo_sms_keyboard())
                return

            elif step == "ademo_interval":
                try:
                    secs = int(float(text.strip()))
                    if secs < 30:
                        raise ValueError()
                except ValueError:
                    bot.send_message(chat_id, "❌ Invalid interval (minimum 30 seconds).", reply_markup=cancel_keyboard())
                    return
                set_setting("demo_sms_interval", secs)
                admin_states.pop(user.id, None)
                bot.send_message(chat_id, _demo_sms_status_text(), reply_markup=demo_sms_keyboard())
                return
        ...
        # ── AUTO SMS DEMO ────────────────────────────────────────────────
        elif text == f"🧪 {stylish('Auto SMS Demo')}":
            bot.send_message(chat_id, _demo_sms_status_text(), reply_markup=demo_sms_keyboard())
            return

        elif text in (f"🧪 {stylish('Demo')}: ON", f"🧪 {stylish('Demo')}: OFF"):
            new_val = "0" if is_demo_sms_enabled() else "1"
            if new_val == "1" and not get_setting("demo_sms_chat_id", ""):
                bot.send_message(
                    chat_id,
                    f"❌ {stylish('Set the Demo Group/Chat ID first.')}",
                    reply_markup=demo_sms_keyboard(),
                )
                return
            set_setting("demo_sms_enabled", new_val)
            bot.send_message(chat_id, _demo_sms_status_text(), reply_markup=demo_sms_keyboard())
            return

        elif text == f"💬 {stylish('Set Demo Group ID')}":
            admin_states[user.id] = {"step": "ademo_chat_id", "data": {}}
            bot.send_message(
                chat_id,
                f"💬 {stylish('Send the demo forward Group/Chat ID (e.g. -1001234567890):')}",
                reply_markup=cancel_keyboard(),
            )
            return

        elif text == f"⏱ {stylish('Set Demo Interval')}":
            admin_states[user.id] = {"step": "ademo_interval", "data": {}}
            bot.send_message(
                chat_id,
                f"⏱ {stylish('Send the demo interval in seconds (minimum 30):')}",
                reply_markup=cancel_keyboard(),
            )
            return
    ...
    if text in (f"🟢 📲 {stylish('Get Number')}", f"🟢 📲 {stylish('Get Another Number')}"):
        ...
        kb.add(InlineKeyboardButton(f"🔵 🛡️ {stylish('Support')}", url=link))
        bot.send_message(chat_id, f"📞 <b>{stylish('Support')}:</b>", reply_markup=kb)
        return
...
def _parse_panel_response(panel_name: str, data: dict, field_map: dict) -> list:
    ...
    for o in otps:
        num = normalize_number(str(o.get(field_map["number"], "") or "").strip())
...
def _poll_zebrasms_success_otps() -> list:
    """Fetch recent OTPs from ZebraSMS GET /publicapi/getupdate (header: MAuth)."""
    cfg = get_api_config("zebrasms")
    if not cfg["enabled"] or not cfg["key"]:
        return []
    try:
        resp = requests.get(f"{ZEBRASMS_BASE}/publicapi/getupdate",
                            headers={"MAuth": cfg["key"]}, timeout=10)
        resp.raise_for_status()
        data = resp.json()
        if isinstance(data, list):
            data = {"data": data}
        logger.info(f"ZebraSMS /getupdate HTTP={resp.status_code} raw={str(data)[:300]}")
        return _parse_panel_response("ZebraSMS", data,
                                     {"number": "number", "message": ["message", "msg", "sms", "text", "full_message"], "otp_id": "otp_id"})
    except Exception as e:
        logger.warning(f"ZebraSMS OTP poll error: {e}")
    return []
...
def _zebrasms_live_access() -> dict:
    """Check ZebraSMS live access status (GET /publicapi/liveaccess, header: MAuth)."""
    cfg = get_api_config("zebrasms")
    if not cfg["key"]:
        return {"ok": False, "detail": "No API key saved"}
    try:
        resp = requests.get(
            f"{ZEBRASMS_BASE}/publicapi/liveaccess",
            headers={"MAuth": cfg["key"]},
            timeout=10,
        )
        ...
        return {"ok": ok, "detail": str(payload)[:200]}
    except Exception as e:
        logger.warning(f"ZebraSMS liveaccess error: {e}")
        return {"ok": False, "detail": str(e)[:200]}


def _fetch_zebrasms_number(rid: str):
    """Allocate from ZebraSMS (POST /publicapi/getnum, header: MAuth, body: {"range": "22501XXX"})."""
    cfg = get_api_config("zebrasms")
    if not cfg["enabled"] or not cfg["key"]:
        return None
    clean_rid   = re.sub(r'[Xx]+$', '', rid)
    ...
    try:
        resp = requests.post(
            f"{ZEBRASMS_BASE}/publicapi/getnum",
            json={"range": range_w_xxx},
            headers={"MAuth": cfg["key"], "Content-Type": "application/json"},
            timeout=10,
        )
        resp.raise_for_status()
        data = resp.json()
        if isinstance(data, dict):
            ...
            if isinstance(payload, list) and payload:
                payload = payload[0]
            if isinstance(payload, dict) and _number_from_api_data(payload):
                return payload
            # Flat shapes: {"number": "..."} / {"full_number": "..."}
            for key in ("number", "full_number", "no_plus_number", "national_number"):
                if data.get(key):
                    return {"full_number": str(data[key])}
    except Exception as e:
        logger.warning(f"ZebraSMS getnum error (rid={rid}): {e}")
    return None
...
def fetch_api_number(rid: str):
    ...
    if not is_standard_range(rid):
        logger.debug(f"Non-standard range '{rid}' — YesMS only")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# AUTO SMS DEMO (synthetic messages only — never real SMS/OTP traffic)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DEMO_SMS_SERVICES = ("Facebook", "Telegram", "WhatsApp", "Instagram", "Google", "TikTok")
DEMO_SMS_COUNTRIES = (
...
def get_demo_sms_interval() -> int:
    try:
        return max(30, int(float(get_setting("demo_sms_interval", 300))))
    except (TypeError, ValueError):
        return 300


def _demo_sms_status_text() -> str:
    state = "🟢 ON" if is_demo_sms_enabled() else "🔴 OFF"
    chat = get_setting("demo_sms_chat_id", "Not set")
    return (
        f"🧪 <b>Auto SMS Demo</b>\n\n"
        f"Status: <b>{state}</b>\n"
        f"💬 Forward Group/Chat ID: <code>{chat}</code>\n"
        f"⏱ Interval: <b>{get_demo_sms_interval()}s</b>\n\n"
        f"<i>All demo messages are synthetic and marked with [DEMO]. "
        f"No real SMS/OTP traffic is captured or forwarded.</i>"
    )


def _build_demo_sms_text() -> str:
    flag, country, code = secrets.choice(DEMO_SMS_COUNTRIES)
    service = secrets.choice(DEMO_SMS_SERVICES)
    number = f"{code}{secrets.randbelow(9_000_000_00) + 1_000_000_00}"
    otp = f"{secrets.randbelow(900000) + 100000}"
    masked = number[:-4] + "****"
    return (
        f"🧪 <b>[DEMO]</b> {stylish('SAMPLE SMS — NOT A REAL OTP')}\n\n"
        f"<blockquote>"
        f"◆ 🌍 {stylish('Country')} ▸ {flag} {country}\n"
        f"◆ 📱 {stylish('Number')}  ▸ <code>{masked}</code>\n"
        f"◆ 🔧 {stylish('Service')} ▸ {service}\n"
        f"◆ 🔑 {stylish('Code')}    ▸ <code>{otp}</code>\n"
        f"◆ 💬 {stylish('Message')} ▸ [DEMO] {otp} is your {service} demo code."
        f"</blockquote>\n"
        f"⚠️ <i>[DEMO] Synthetic demo message generated by the bot.</i>"
    )


def demo_sms_worker():
    """Periodically post synthetic [DEMO] SMS cards to the configured group."""
    logger.info("Auto SMS Demo worker started.")
    while True:
        try:
            if is_demo_sms_enabled():
                target = str(get_setting("demo_sms_chat_id", "") or "").strip()
                if target:
                    bot.send_message(target, _build_demo_sms_text())
                else:
                    logger.warning("Auto SMS Demo enabled but no chat ID configured.")
        except Exception as e:
            logger.warning(f"Auto SMS Demo send error: {e}")
        time.sleep(get_demo_sms_interval() if is_demo_sms_enabled() else 30)
...
if __name__ == "__main__":
    ...
    demo_sms_thread = threading.Thread(target=demo_sms_worker, daemon=True, name="demo-sms")
