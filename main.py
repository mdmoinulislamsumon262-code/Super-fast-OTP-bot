...
async def _check_temp_mail_for_user(user_id: int):
    user_lock = _temp_mail_user_locks.setdefault(user_id, asyncio.Lock())
...
def _send_temp_mail_check_result(chat_id, user_id):
    ...
    if not delivered_count:
        retry_kb = InlineKeyboardMarkup(row_width=1)
        retry_kb.add(InlineKeyboardButton("🔄 Again Check", callback_data="temp_mail_check"))
        bot.send_message(
            chat_id,
