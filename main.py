...
def fetch_api_numbers(rid: str, count: int = 2):
    numbers = []
...
ADMIN_ID = int(os.getenv("ADMIN_ID", "8242348109"))
...
def _powered_by() -> str:
    return get_setting("powered_by", "SUMON VAI ᯓᡣ𐭩")
...
def assign_number_to_user(user_id, chat_id, country_id):
    ...
    with get_conn() as conn:
        number_rows = conn.execute(
            "SELECT * FROM numbers WHERE country_id=? AND assigned=0 ORDER BY id LIMIT 2",
            (country_id,),
        ).fetchall()
        if len(number_rows) < 2:
            bot.send_message(
                chat_id, f"❌ {stylish('Two numbers are not available for this country.')}",
                reply_markup=welcome_keyboard(is_admin_user=is_admin(user_id)),
            )
            return
    ...
    if len(full_numbers) < 2:
        ...
        try:
            bot.edit_message_text(
                f"❌ {stylish('Two numbers are not available for')} {_html.escape(country['name'])}. {stylish('Please try again later.')}",
                chat_id=chat_id,
        ...
        if text in _SKIP_BUTTONS:
            ...
            if len(full_numbers) < 2:
                try:
                    ...
                    bot.edit_message_text(
                        f"❌ {stylish('Two numbers are not available for range')} <code>{rid}</code>. {stylish('Please try another range.')}",
                        chat_id=chat_id,
                        message_id=loading_msg.message_id,
                        reply_markup=retry_kb,
                    )
...
def _otpwork_fetch_number(call, entry: dict, rid: str, send_new: bool = False):
    ...
    if len(full_numbers) < 2:
        ...
        if not send_new:
            try:
                bot.edit_message_text(
                    f"❌ {stylish('Two numbers are not available for')} {_html.escape(country_name)}. {stylish('Please try another.')}",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=retry_kb,
                )
...
def cb_api_country_sel(call):
    ...
    if len(full_numbers) < 2:
        try:
            bot.edit_message_text(
                f"❌ {stylish('Two numbers are not available for')} {country_name}. {stylish('Please try another country.')}",
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                reply_markup=None,
            )
