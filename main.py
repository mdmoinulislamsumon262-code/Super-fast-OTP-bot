...
def auto_sms_inline_keyboard():
    """Buttons under an Auto SMS card — panel link + channel/group link."""
    panel_link = normalize_link(get_setting("panel_link", ""))
    otp_link = normalize_link(get_setting("otp_group_link", ""))
...
def _zebra_request(method: str, paths, key: str, json_body=None, timeout: int = 12):
...
def _fetch_zebrasms_number(rid: str):
    ...
    for body in bodies:
        data, _code, _h, _p = _zebra_request("POST", ZEBRA_NUM_PATHS, cfg["key"], json_body=body)
